...
public class Shell {
	...
	/**
	 * A class containing automatically created shell attempts and links to both {@link Shell#(String[], Integer[], OnShellResultListener)} and {@link Shell#(String[], Integer[])} <br /><br />
	 * 
	...
	public Shell(Boolean requestRoot) {
		...
		for (int i=0; i < 2; i++) {
			...
			mStream = new ShellStream(requestRoot, new ShellStream.OnStreamListener() {
				@Override
	 ...
	 * 
	 * @see Shell#(String[], Integer[], OnShellResultListener)
	 * 
	 * @param command
	 *     The command to execute
	 * 
	...
	/**
	 * Locate whichever toolbox in {@value } that supports a specific command.<br /><br />
	 * 
	 ...
	 */
	public Filesystem.Disk getDisk(String disk) {
		return new Filesystem.Disk(this, disk);
	}
	 ...
	 *     The /dev/ swap device
	 */
	public Memory.Swap getSwap(String device) {
		return new Memory.Swap(this, device);
	}
}
