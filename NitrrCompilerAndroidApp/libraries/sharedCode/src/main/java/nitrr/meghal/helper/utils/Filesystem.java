...
public class Filesystem {
			...
			if (oFstabList == null) {
				...
				if (result != null && result.wasSuccessful()) {
					...
					for (int i=0; i < dirs.length; i++) {
						...
						File.FileData data = mShell.getFile(dirs[i]).readMatch( (isFstab ? "/dev/" : "mount "), false );
									...
									try {
										...
										if (parts.length > 3 && !cache.contains(parts[ isFstab ? 1 : 3 ])) {
											if (!isFstab && parts[2].contains("mtd@")) {
												...
												File.FileData mtd = mShell.getFile("/proc/mtd").readMatch( ("\"" + parts[2].substring(4) + "\""), false );
	...
	public MountStat[] getMountList() {
		File.FileData data = mShell.getFile("/proc/mounts").read();
