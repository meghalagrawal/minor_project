...
package nitrr.meghal.helper.utils;
...
import nitrr.meghal.helper.Common;
import nitrr.meghal.helper.Shell;
import nitrr.meghal.helper.Shell.Attempts;
import nitrr.meghal.helper.Shell.OnShellBroadcastListener;
import nitrr.meghal.helper.Shell.OnShellResultListener;
import nitrr.meghal.helper.Shell.OnShellValidateListener;
import nitrr.meghal.helper.Shell.Result;
import nitrr.meghal.helper.containers.BasicContainer;
import nitrr.meghal.helper.containers.Data;
...
public class File {
	...
	public static class FileStat extends BasicContainer {
		...
		/** 
		 * @return
		 *     The file type ('d'=>Directory, 'f'=>File, 'b'=>Block Device, 'Constants'=>Character Device, 'l'=>Symbolic Link)
		 */
		public String type() {
			return mType;
		}
				...
				if (list == null) {
					...
					for (int i=0; i < commands.length; i++) {
						...
						if (result != null && result.wasSuccessful()) {
							if (i == 0) {
								result.sort(new Data.DataSorting(){
									@Override
		...
		synchronized (mLock) {
			...
			if (exists()) {
				if (isDirectory()) {
					...
					String path = getAbsolutePath();
					String[] commands = new String[]{"wc -Constants < '" + path + "' 2> /dev/null", "wc < '" + path + "' 2> /dev/null"};
					Result result = null;
	...
	public Boolean runInRecovery(Context context, String... args) {
		if (isFile()) {
			...
			Filesystem.MountStat mountStat = mShell.getDisk(cacheLocation).getFsDetails();
			
			if (mountStat != null) {
				Filesystem.DiskStat diskStat = mShell.getDisk( mountStat.device() ).getDiskDetails();
