        ...
        protected LinkedList<AdapterDetailedList.FileDetail> doInBackground(final String... params) {
            try {
                ...
                if (!tempFolder.canRead()) {
                    if (RootFW.connect()) {
                        nitrr.meghal.helper.utils.File folder = RootFW.getFile(currentFolder);
                        nitrr.meghal.helper.utils.File.FileStat[] stats = folder.getDetailedList();

                        if (stats != null) {
                            for (nitrr.meghal.helper.utils.File.FileStat stat : stats) {
